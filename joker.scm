(define (best-total sent)
  (define (total sent)
    (cond ((empty? sent) 0)
        ...
        ((member? (first (first sent)) '(r)) (+ 1 (total (bf sent))))
        ((= (count (first sent)) 3) (+ 10 (total (bf sent))))
        (else (+ (first (first sent)) (total (bf sent))))))
(joker (ace-value (total sent) sent) sent))

(define (joker val hand)
  (cond ((and (> val 21) (or (= (jokerful hand) 0) (= (jokerful hand) 1) (= (jokerful hand) 2))) val)
  ((and (>= val 1) (<= val 10) (= (jokerful hand) 1)) (+ val 10))
  ((and (>= val 1) (<= val 10) (= (jokerful hand) 2)) 21)
  ((and (>= val 11) (<= val 20) (or (= (jokerful hand) 1) (= (jokerful hand) 2))) 21)
  (else val)))

(define (jokerful hand)
  (cond ((empty? hand) 0)
    ((equal? 'r (first (first hand))) (+ 1 (jokerful (bf hand))))
    (else (jokerful (bf hand)))))
...
(define (make-ordered-deck)
  ...
  (se (make-suit 'H) (make-suit 'S) (make-suit 'D) (make-suit 'C) 'R 'R) )
